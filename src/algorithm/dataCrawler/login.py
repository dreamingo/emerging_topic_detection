...
def get_code(url, APP_KEY):
    conn = httplib.HTTPSConnection('api.weibo.com')
    postdata = urllib.urlencode ({
        'client_id':APP_KEY,
        'response_type':'code',
        'redirect_uri':configure.CALLBACK_URL,
        'action':'submit',
        'userId':configure.ACCOUNT,
...
def APILogin(id):
    try:
        file_ptr = open("../data/appkey.dat", "r")
        Appkey = cPickle.load(file_ptr)
        if id >= len(Appkey):
            id = 0
        APP_KEY = Appkey[id]['app_key']
        APP_SECRET = Appkey[id]['app_secret']
        client  = APIClient(app_key=APP_KEY, app_secret = APP_SECRET, redirect_uri=configure.CALLBACK_URL)
        url = client.get_authorize_url()

        r = client.request_access_token(get_code(url, APP_KEY))
        client.set_access_token(r.access_token, r.expires_in)
        print "Succefully APILogin"
        return  client
...
if __name__ == '__main__':
	...
	kaifu_page = urllib2.urlopen('http://weibo.com/1957829952/profile?topnav=1&wvr=5').read()
	print kaifu_page
